...
export async function csrfFetch(url, options = {}) {
  ...
  // call the default window's fetch with the url and the options passed in and set to variable res
  const res = await window.fetch(url, options);

  // if the response (res) status code is 400 or above, then throw an error with the error being the response
